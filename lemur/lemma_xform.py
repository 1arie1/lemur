...
def parse_lemma_xform_calls(trace_path: str | Path) -> list[NraCall]:
...
def _calls_from_entries(entries: list[TraceEntry]) -> list[NraCall]:
    ...
    for lemma, vm_entry in zip(lemmas, varmaps):
        ...
        calls.append(NraCall(
            index=len(calls),
            constraints=tuple(sorted(resolved)),
...
def parse_xform_calls(
    ...
    if prefer == 'varmap':
        return parse_lemma_xform_calls(trace_path), 'varmap'
    if prefer == 'nra':
        src = nra_trace_path if nra_trace_path else trace_path
        return parse_nra_calls(src), 'nra'

    # auto
    calls = parse_lemma_xform_calls(trace_path)
    if calls:
        return calls, 'varmap'
    src = nra_trace_path if nra_trace_path else trace_path
    return parse_nra_calls(src), 'nra'
